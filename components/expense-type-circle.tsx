...
const TYPE_STYLE: Record<
  ExpenseType,
  { letter: string; className: string; labelKey: string }
> = {
  empresa: {
    letter: "E",
    labelKey: "type.empresa",
    className:
...
export function ExpenseTypeLegend({ className = "" }: { className?: string }) {
  ...
  return (
    ...
    >
      {types.map((ty) => (
        <span key={ty} className="inline-flex items-center gap-2" role="listitem">
          <ExpenseTypeCircle type={ty} size="sm" />
          <span className="text-pin-ink/80">{t(TYPE_STYLE[ty].labelKey)}</span>
        </span>
      ))}
    </div>
  );
